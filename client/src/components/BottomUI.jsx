            ...
            <div className='curve bg-gradient-to-t from-yellow-900 via-yellow-700 to-yellow-900 flex items-center border-t-4 border-yellow-900 shadow-[0_35px_20px_35px_rgba(0,0,0,1)]'>
                ...
                {(lobbyData?.phase === 'playing' && lobbyData?.seats[props.seatNumber]?.isTurn && !lobbyData?.seats[props.seatNumber]?.isBusted)
                    ...
                    <div className='flex gap-12 w-1/3 justify-center p-4'>
                        ...
                        <button onClick={() => onClickHit(props.seatNumber)} className='border-2 border-black rounded-full w-20 bg-red-700 hover:bg-red-600 shadow-black shadow-md  font-bold transition-all duration-200'><img className='w-10 mx-auto' src={addCard} alt='AddCardIcon' />HIT</button>
                ...
                <div className='flex w-1/3 gap-4 justify-center bg-green-800 shadow-inner shadow-black rounded-full rounded-r-none px-12 py-2'>
                    <button disabled={lobbyData?.phase !== 'betting'} onClick={() => onClickBet(5)} className='w-16 drop-shadow-[0_8px_8px_rgba(0,0,0,0.8)] disabled:opacity-50'><img src={Chip5} alt='Chip5' /></button>
                    <button disabled={lobbyData?.phase !== 'betting'} onClick={() => onClickBet(10)} className='w-16 drop-shadow-[0_8px_8px_rgba(0,0,0,0.8)] disabled:opacity-50'><img src={Chip10} alt='Chip10' /></button>
                    <button disabled={lobbyData?.phase !== 'betting'} onClick={() => onClickBet(20)} className='w-16 drop-shadow-[0_8px_8px_rgba(0,0,0,0.8)] disabled:opacity-50'><img src={Chip20} alt='Chip20' /></button>
                    <button disabled={lobbyData?.phase !== 'betting'} onClick={() => onClickBet(50)} className='w-16 drop-shadow-[0_8px_8px_rgba(0,0,0,0.8)] disabled:opacity-50'><img src={Chip50} alt='Chip50' /></button>
                    <button disabled={lobbyData?.phase !== 'betting'} onClick={() => onClickBet(100)} className='w-16 drop-shadow-[0_8px_8px_rgba(0,0,0,0.8)] disabled:opacity-50'><img src={Chip100} alt='Chip100' /></button>
                    <button disabled={lobbyData?.phase !== 'betting'} onClick={() => onClickBet(500)} className='w-16 drop-shadow-[0_8px_8px_rgba(0,0,0,0.8)] disabled:opacity-50'><img src={Chip500} alt='Chip500' /></button>
                </div>
            </div>
        </>
